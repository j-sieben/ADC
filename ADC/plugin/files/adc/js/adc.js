...
(function (adc, $, server) {
  ...
  var triggeringElement = {
          ...
          "isClick": false};
  var pageStatus = {};
      pageStatus.itemMap = new Map();

  /* ++++++++++ PRIVATE +++++++++ */
  /**
    Group: Private Callback Methods
   */
  /** 
    ...
    Parameters:
      ...
      pEventData - Additional event data that is passed with the event. May be accessed by ADC.GET_EVENT_DATA within the database
  ...
  /** 
    Function: confirmCallback
      ...
      Delegates showing the confirmation dialog to <adc.ApexJS>.
  ...
  /** 
    ...
    Parameters:
      pEvent - Event that occured
      pEventData - <eventData> instance that goes along with the event
   */
  ...
  function getRegionType(pRegionId){
    ...
    var C_TAB_SELECTOR = `#SR_${pRegionId}`;
  ...
  function hasUnsavedChanges(pPageItems){
    ...
    $.each(itemList, function(item){
      ...
      if (pageStatus.itemMap.has(item) && pageStatus.itemMap.get(item) != apex.item(item).getValue()){
        isDifferent = true;
        return false;
      }
    });
   ...
   * To achive this, parameter <pAction> with the necessary attributes is passed in and evaluated.<br>
  ...
  /**
   * Sets vertical alignment of IR and IG to top. Delegates aligning to <adc.ApexJS>.
  ...
  /**
    Function: adc.confirmAndExecuteCommand
   ...
   */
  adc.confirmAndExecuteCommand = function (pMessage, pData, pItem){
    triggeringElement.event = 'command';
    triggeringElement.id = 'COMMAND';
    triggeringElement.isClick = false;    
    // Handle event only after confirmation from the user
    adc.ApexJS.confirmRequest(pEvent, function() {adc.executeCommand(pData);});
  };

  /**
    Function: executeCommand
      Wrapper around <adc.execute> that raises a command event along with the necessary data.
  ...
  adc.executeCommand = function(pData){
    ...
    if(typeof pData == 'string'){
      data = {"command":pData, "additionalPageItems":[], "monitorChanges": false};
    }
    else{
      data = pData;
    ...
    if(pData.monitorChanges && hasUnsavedChanges()){
        // Unsaved changes among the observed page items, check with user
        event.data = pData;
        event.data.message = pageStatus.message;
        event.data.title = pageStatus.title;
        adc.ApexJS.confirmRequest(event, adc.execute);      
    }else{
      adc.execute();
    }
  };
  ...
  /**
    Function: adc.getReportSelection
   ...
   */
  adc.focus = function(pItemId){
    $(`#${pItemId}`).focus();
  };
  
  /**
    Function: adc.hideReportFilterPanel
      Hides filter panels from IR and IG. Delegates hiding the filter panel to <adc.ApexJS>.
  ...
  /**
   * Method to show a notification to the user. Delegates implementation to <adc.ApexJS>.<br>
  ...
  function pauseChangeEventDuringRefresh(pItemId, pItemValue){
    var $item = $(`#${pItemId}`);
    var node = $item.get(0);
    var itemEvents;
    var temporalEvents;
    ...
    if ($item.length > 0){
      ...
      .one(C_APEX_AFTER_REFRESH, function(e){
        if (pItemValue){
          ...
          // if we are observing this item for changes, update the value to prevent fals true change messages
          if (pageStatus.itemMap.has(pItemId)){
            pageStatus.itemMap.set(pItemId, pItemValue);
          };
        }; 
   ...
   * @public
   */
  adc.rememberPageItemStatus = function(pPageItems, pMessage, pTitle){
    var itemList;
    var itemValue;
    
    // Initialize
    pageStatus.itemMap.clear();
    pageStatus.message = pMessage;
    pageStatus.title = pTitle;
    ...
    $.each(itemList, function(item){
        ...
        pageStatus.itemMap.set(item, itemValue);
   ...
   * <strong>IMPORTANT>strong>: This method only supports simple shortcuts like <Alt-T>!
   ...
   * Delegates showing the errors to <adc.ApexJS>.<br>
   ...
   * while the focus is still in the input field, two events will be raised: <change> on the input field and <click> on the button.<br>
   * Now, ADC may validate then input field and the <click> event should only be processed if the validation passes.
   * As both events are raised (almost) concurrently and handled asynchronously, there is no possibility for ADC to prevent the <click> event from happening.
   * To cater for this, all events are queued within ADC and therefore serialized. Using this technique, the <click> event can be surpressed by clearing the queue.
   ...
   * @param {Object} pPageItems Array of objects of the form <{"id":"pageItemID","value":"itemValue"}>.
   */
   ...
   * Controling the page representation is delegated to <adc.ApexJS>.
   ...
   */
  adc.setRegionContent = function(pRegionId, pContent){
    $(`#${pRegionId} .t-Region-body`).html(pContent);
  }; // setRegionContent
  
  
  /**
    Function: adc.setRegionHeader
   ...
   */
  adc.setRegionHeader = function(pRegionId, pHeader){
    switch(getRegionType(pRegionId)){
      case C_REGION_TAB:
        $(`#SR_${pRegionId}_tab a span`).html(pHeader)
        break;
      default:
        $(`#${pRegionId} .t-Region-title`).html(pHeader);
        break;
    }
  }; // setRegionHeader

  /*
    Function: adc.selectTab
   ...
   */
  adc.selectTab = function(pTabRegionId, pTabId){
