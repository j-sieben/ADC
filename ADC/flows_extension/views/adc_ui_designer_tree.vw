...
with page_state as(
       select /*+ no_merge */
              coalesce(utl_apex.get_number('CRG_APP_ID'), 117) p_app_id,
     ...
     data as(
       select /*+ no_merge (page_state) */
              'CRG_' || crg_id node_id, 'PAGE' node_parent_id, crg_page_id || ': ' || page_name || ' (' || page_alias || ')' node_name, crg_page_id node_sort_seq, icon_cgr || color_cgr node_icon
       ...
       select 'CAA_' || caa_id, 'CAG_' || caa_crg_id, caa_label || ' (' || caa_name || ')', caa_id, icon_caa|| color_caa
         ...
         join page_state
           on crg_app_id = p_app_id
        union all
       select 'CRU_' || cru_id, 'CRG_' || cru_crg_id, cru_name, cru_sort_seq, icon_cru || case cru_active when c_true then color_cru else color_deactive end
         from adc_rules
         join adc_rule_groups
