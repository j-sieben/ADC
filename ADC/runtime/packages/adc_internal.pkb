...
as
  ...
  C_MANDATORY_COLLECTION_NAME constant adc_util.ora_name_type := C_PARAM_GROUP || '_CGR_STATUS_';
  ...
  -- Recursion stack
  -- The recursion stack stores the page items that were changed by the rules
  -- to subsequently call the rule check for these items as well.
  ...
  /** Get message text from PIT to integrate it as a comment into the response
  ...
  /** Method to retrieve a default value for a manadatory item
  ...
  begin
    pit.enter_optional('get_mandatory_default',
      p_params => msg_params(
   ...
   * @param  p_cpi_mandatory_message  Optional explicit error message
  ...
  begin
    pit.enter_optional('get_mandatory_message',
      p_params => msg_params(
                    msg_param('p_cpi_id', p_cpi_id),
                    msg_param('p_cpi_mandatory_message', limit_string(p_cpi_mandatory_message))));
                    
    if p_cpi_mandatory_message is null and p_cpi_id != adc_util.C_NO_FIRING_ITEM then
      ...
      l_mandatory_message := p_cpi_mandatory_message;
    end if;
  ...
  /** Method to register a page item at the list of firing items
   * @param  p_cpi_id  ID of the item to add to the firing items list
   * @usage  The list of firing items collects all page items which are touched by the rules evaluated.
   *         We need to maintain this list in order to harmonize their session status with the client
   *         and to remove any errors of these items prior to attaching new errors
   */
  ...
  begin
    ...
    l_item_value := trim(coalesce(utl_apex.get_value(p_cpi_id), get_mandatory_default(p_cpi_id)));
   ...
   * TODO; Refactor jQuery to parameter type JQUERY to stabilize its detection
   */
   ...
   * TODO: Consider removing the recursive flag from the rules. Probably not needed anymore
   */
  ...
  begin
    ...
    -- Extract JavaScript chunk, replace parameters and register with response unless PL_SQL code has set the STOP flag
    if p_rule.js is not null and not g_param.stop_flag then
  ...
  as
    l_rule rule_rec;
    l_rule_name adc_rules.cru_name%type;
  ...
  $END
    ...
    if not l_has_js then
      ...
      add_comment(
        ...
        p_msg_args => msg_args(l_rule_name, adc_util.C_CR));
    end if;
  ...
  begin
    ...
    l_page_items := join_list(g_param.page_items);
  ...
  begin
    ...
    g_param.collection_name := C_MANDATORY_COLLECTION_NAME || g_param.cgr_id;
  ...
  begin
    ...
    l_char := coalesce(v(p_cpi_id), get_mandatory_default(p_cpi_id));
  ...
  begin
    ...
    l_format_mask := coalesce(P_format_mask, get_conversion(p_cpi_id));
  ...
  begin
    ...
    l_format_mask := replace(coalesce(p_format_mask, get_conversion(p_cpi_id)), 'G');
    ...
    when p_cpi_id != adc_util.C_NO_FIRING_ITEM then
      ...
      case when p_is_mandatory = adc_util.C_TRUE and l_srs_row.srs_id is null then
        apex_collection.add_member(
          ...
          p_c003 => p_cpi_mandatory_message,
          p_generate_md5 => 'NO');
  ...
  begin
    ...
    if p_cpi_id = adc_util.c_no_firing_item or p_cpi_id is null then
      ...
      -- Parse SQL to determine column identifiers
      ...
      -- Execute SQL and load first row
      l_cnt := dbms_sql.execute_and_fetch(l_cur);
      -- Copy all column values to page items with corresponding column name
      for i in 1 .. l_col_cnt loop
        dbms_sql.column_value(l_cur, i, l_result);
        -- Copy value to session state
        -- TODO: Stabilize this behavior
        set_session_state(utl_apex.get_page_prefix || l_desc_tab(i).col_name, l_result);
      end loop;
      dbms_sql.close_cursor(l_cur);
    else
      -- Concrete item requested, according to convention only one column is included
  ...
  as
    ...
    l_errors_occured boolean default false;
  begin
    -- Tracing done in ADC_API
    ...
    for itm in mandatory_item_cur loop
      ...
      if get_string(itm.srs_cpi_id) is null then
        ...
        register_error(itm.srs_cpi_id, get_mandatory_message(itm.srs_cpi_id, itm.srs_cpi_mandatory_message), '');
        l_errors_occured := true;
      end if;
    end loop;
