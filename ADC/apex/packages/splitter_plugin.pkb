...
as
  ...
  -- check whether p_str is numeric
  function is_numeric(
    p_str varchar2)
  return boolean
  as
  ...
  begin
    ...
    if is_numeric(p_position) and p_collapsed in ('true', 'false') then
  ...
  begin
    ...
    -- Initialization
    l_init_js_fn := nvl(apex_plugin_util.replace_substitutions(p_region.init_javascript_code), 'undefined');
