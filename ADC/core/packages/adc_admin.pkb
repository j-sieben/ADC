...
as
  ...
  /**
    Procedure: harmonize_firing_items
      ...
      Method extracts page item names from a rule condition using C_REGEX_ITEM.
  ...
  begin
    ...
    l_view_stmt := rtrim(adc_parameter.get_param_lov_query(p_row), ';');
    ...
    using (select p_row.capt_id capt_id,
     ...
     when matched then update set
          ...
          t.capt_select_list_query = s.capt_select_list_query,
          t.capt_select_view_comment = s.capt_select_view_comment
     when not matched then insert(
            capt_id, capt_pti_id, capt_pmg_name, capt_capvt_id,
            capt_select_list_query, capt_select_view_comment, capt_sort_seq, capt_active)
          values(
            s.capt_id, s.capt_pti_id, s.capt_pmg_name, s.capt_capvt_id,
            s.capt_select_list_query, s.capt_select_view_comment, s.capt_sort_seq, s.capt_active);
  ...
  begin
    ...
    l_zip_file_name := param.get_string('ACTION_TYPE_FILENAME', C_ADC);
  ...
  begin
    ...
    adc_config.merge_apex_action(l_row);
  ...
  begin
    ...
    adc_config.merge_apex_action_item(l_row);
  ...
  /**
    Group: Deprecated Config Wrappers
      Compatibility wrappers for previously exported movement-data scripts.
      New code must use ADC_CONFIG directly.
      For documentation on these methods, see ADC_CONFIG.
   */
  function map_id(
    p_id in number)
    return number
  ...
  begin
    ...
    adc_config.merge_rule_group(l_row);
  ...
  begin
    ...
    adc_config.delete_rule_group(l_row);
  ...
  begin
    ...
    adc_config.propagate_rule_change(p_crg_id);
    pit.leave_mandatory;
  ...
  begin
    ...
    adc_config.prepare_rule_group_import(p_workspace, p_app_alias);
    pit.leave_mandatory;
  ...
  begin
    ...
    adc_config.prepare_rule_group_import(p_workspace, p_app_id);
    pit.leave_mandatory;
  end prepare_rule_group_import;
  ...
  begin
    ...
    adc_config.prepare_rule_group_import(p_crg_app_id, p_crg_page_id);
    pit.leave_mandatory;
  end prepare_rule_group_import;
  ...
  begin
    ...
    adc_config.merge_rule_action(l_row);
