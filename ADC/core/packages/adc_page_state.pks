...
as
  ...
  /**
    Function: get_page_items_as_json
      Method to calculate a list of page items and return the list as JSON array
      
    Parameters:
      p_cpi_id - ID of the document. If adc_util.C_NO_FIRING_ITEM then
                 P_PAGE_ITEMS is interpreted as a jQuery selector
      p_page_items - List of items to get or a jQuery selector.
  
    Returns:
      JSON array with all selected page items
   */
  function get_page_items_as_json(
  ...
  /**
    ...
    Parameter:
      p_crg_id - ID of the rule group. Is used to be cached to avoid function calls
      p_cpi_id - ID of the firing item. Is used to harmonize the actual value with the
                 session state, as this value may have to be changed during rule execution
   */
  procedure reset(
