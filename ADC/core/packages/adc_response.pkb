...
as
  ...
  */
  ...
  C_JS_COMMENT_START constant adc_util.sql_char := '/* ';
  C_JS_COMMENT_END constant adc_util.sql_char := ' */';
  C_COMMENT_OUT constant varchar2(20) := C_JS_COMMENT_START || '(double)' || C_JS_COMMENT_END;
  ...
  /**
    Type: error_hash_stack_t
      Stack to store error hashes
   */
  type error_hash_stack_t is table of binary_integer index by binary_integer;
  ...
  type param_rec is record(
    ...
    error_hash_stack error_hash_stack_t,
  ...
  begin
    ...
    g_param.error_hash_stack.delete;
  ...
  begin
    ...
    if p_java_script is not null then
      ...
      if p_debug_level not in (adc_util.C_JS_CODE, adc_util.C_JS_RULE_ORIGIN) then
        l_response.script := ltrim(rtrim(l_response.script, C_JS_COMMENT_START), C_JS_COMMENT_END);
        l_response.script := C_JS_COMMENT_START || l_response.script || C_JS_COMMENT_END;
      end if;
  ...
  begin
    ...
    if pit.check_log_level_greater_equal(l_message.severity) then
      add_javascript(C_JS_COMMENT_START || l_message.message_text || C_JS_COMMENT_END, l_message.severity);
    end if;
  ...
  as
    C_MAX_INTEGER constant number := 2147483647; -- Used to limit hash function to the max of BINARY_INTEGER
  ...
  begin
    ...
    if not g_param.error_hash_stack.exists(l_error_hash) then
      g_param.error_hash_stack(l_error_hash) := g_param.error_hash_stack.count + 1;
      ...
      g_param.error_stack(g_param.error_hash_stack(l_error_hash)) := l_error_json;
    end if;
