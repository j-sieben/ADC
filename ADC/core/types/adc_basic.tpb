...
as
  ...
  as
    ...
    l_javascript adc_util.max_char := q'^de.condes.plugin.adc.actions.confirmCommand ('#MESSAGE#', '#COMMAND#', '#FOCUSITEM#')^';
  begin
    pit.enter_optional(
      p_params => msg_params(
                    msg_param('p_focus_item', p_focus_item),
                    msg_param('p_command', p_command),
                    msg_param('p_message_name', p_message_name)));

    l_message := pit.get_message(p_message_name, p_msg_args);

    l_javascript := REPLACE(REPLACE(REPLACE(l_javascript,
                      '#MESSAGE#', l_message.message_text),
                      '#COMMAND#', p_command),
                      '#FOCUSITEM#', p_focus_item);

    adc_api.add_javascript(l_javascript);
  ...
  begin
    ...
    adc_api.exclusive_or(p_cpi_id, p_value_list, p_message, p_error_on_null = adc_util.C_TRUE);
  ...
  as
  begin
    return adc_api.get_string(p_cpi_id) = adc_util.C_TRUE;
  end get_flag;
